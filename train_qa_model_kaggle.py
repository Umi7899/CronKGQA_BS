...
parser.add_argument(
    '--tkbc_model_file', default='tcomplex_17dec.ckpt', type=str,
...
def train(qa_model, dataset, valid_dataset, args,result_filename=None):
    ...
    if result_filename is None:
        result_filename = '/kaggle/working/results/{dataset_name}/{model_file}.log'.format(
            dataset_name = args.dataset_name,
            model_file = args.save_to
        )
    checkpoint_file_name = '/kaggle/working/models/{dataset_name}/qa_models/{model_file}.ckpt'.format(
        dataset_name = args.dataset_name,
        model_file = args.save_to
    )
